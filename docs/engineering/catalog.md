...
| `extractWhatsmiauConnectQr` / `asWhatsAppQrDataUri`                        | `server/whatsmiau.ts`                                           | Extrair e normalizar o QR de pareamento retornado por `/instance/connect` (string, `base64` aninhado ou top-level) para data URI PNG. Usar no reconnect; não buscar só `/image` sem iniciar o connect.                                                                              |
