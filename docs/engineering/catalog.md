...
| `SupabaseMcpConnectionAdapter.runtimeList`            | `server/supabase-api-adapters.ts`                               | Carregar conexões MCP prontas para o worker, incluindo headers criptografados ou token OAuth renovado. Workers não devem consultar ou descriptografar `mcp_connection_secrets` diretamente.                                                     |
...
| `resolveEffectiveRunConfig` / `ResearchArtifact`      | `server/coding-control-plane.ts`                                | Resolver precedência de rota, validar catálogo/capabilities, congelar a configuração da run e content-addressar a pesquisa. Implementações devem referenciar o artefato; não duplicar pesquisa repo-wide.                                       |
