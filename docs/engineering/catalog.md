...
| `ConversationPendingActionStore`                                                   | `server/adapters/supabase/pending-actions.ts`                   | Persistir ações pendentes Sim/Não por conversa (Zelinho-style). Uma `pending` por conversa; expiração e idempotência no worker.                                                                                                                                                                          |
| `runZeloPdvGerenteBridge` / confirm-words / catalog-intent                         | `server/integrations/zelopdv-gerente/`                          | Encaminhar mutações de catálogo ao `/api/gerente/channel` do ZeloPDV e resolver confirmações Sim/Não antes da triagem LLM. Não executar SQL de delete no Mend.                                                                                                                                         |
...
| `SupportEvidenceBundle` / `validateGroundedSupportReply`                           | `server/support-evidence.ts`                                    | Entregar ao provider fatos limitados e chaves opacas. Validar citações e impedir detalhes internos na resposta ao cliente; persistir a evidência usada separadamente do corpo enviado.                                                                                                                 |
