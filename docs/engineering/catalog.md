...
| `SupabaseChannelAdapter.refresh` / `remove`                                | `server/adapters/supabase/messaging.ts`                         | Refresh tolerante a instância ausente (404 → `closed`); `remove` apaga no Whatsmiau e em `channel_connections`. Settings não deve marcar o WhatsApp inteiro como indisponível por um refresh falho.                                                                               |
