...
| `extractWhatsmiauConnectQr` / `asWhatsAppQrDataUri` / `normalizeMessagingInstance` | `server/whatsmiau.ts`                                           | Extrair e normalizar o QR de pareamento retornado por create/connect (string, `base64` aninhado, top-level ou `data`) para data URI PNG. Usar no create e reconnect; não buscar só `/image` sem iniciar o connect.                                                                                     |
| `SupabaseChannelAdapter.refresh` / `remove`                                        | `server/adapters/supabase/messaging.ts`                         | Refresh tolerante a instância ausente (404 → `closed`); `remove` exclui no Whatsmiau (`/instance/delete`) e só então apaga `channel_connections`. Falha no provedor → `whatsapp_provider_delete_failed` (não orphanar Cloud). Settings não marca WhatsApp inteiro como indisponível por refresh falho. |
| Whatsmiau `/instance/create` payload                                               | `server/whatsmiau.ts`                                           | Enviar só `instanceName` por padrão. Não mandar `qrcode` (fora do schema Cloud) nem `syncFullHistory` sem slot pago. Parear com `/instance/connect` depois do create.                                                                                                                                  |
| `whatsmiauApiHttpError`                                                            | `server/api-router.ts`                                          | Mapear falhas de assinatura/billing do Whatsmiau (`subscription is not active`, `billing_*`, HTTP 423) para `whatsapp_subscription_inactive` / `whatsapp_instance_suspended`. Create/connect não funcionam sem assinatura Cloud ativa na conta da `WHATSMIAU_API_KEY`.                                 |
| `mergeConversationSnapshot` / `stripHumanWhatsAppIntro`                            | `src/features/inbox/conversation-snapshot.ts`                   | Merge de thread: `partialMessages` (lista do Inbox) só faz upsert da bolha recente e **nunca encolhe** histórico hidratado; snapshot completo mantém páginas antigas locais; aposenta `temp:` ignorando intro WhatsApp.                                                                                |
| `loadLiveConversationSnapshot`                                                     | `src/api/live-actions.ts` + Inbox                               | Ao abrir um thread, hidratar até 50 mensagens recentes. A lista do Inbox só embute a última mensagem por conversa; sem o snapshot o canvas fica com um único balão.                                                                                                                                    |
...
| `resolveSupportProducts`                                                           | `server/knowledge-products.ts`                                  | Resolver produtos antes do retrieval. Nomes explícitos e aliases decisivos podem persistir contexto; termos genéricos ou resultados próximos permanecem ambíguos e bloqueiam auto-send.                                                                                                                |
| `extractRepositoryKnowledge`                                                       | `server/knowledge-source-extractor.ts`                          | Extrair somente fontes textuais allowlisted de um checkout exato e read-only. Aplicar exclusões sensíveis, limites por arquivo/execução e nunca executar código do repositório.                                                                                                                        |
| `SupportEvidenceBundle` / `validateGroundedSupportReply` / `sanitizeCustomerSupportReply` | `server/support-evidence.ts` | Entregar ao provider fatos limitados e chaves opacas. Remover playbooks de escalonamento (“Quando encaminhar para humano”) antes do envio; validar citações e impedir vazamento interno na resposta ao cliente. |
| `resolveAiReplyChoices` / `sendReplyChoices` | `server/automation/reply-choices.ts`, `server/whatsapp-service.ts` | Clarificações com 2–3 opções viram reply buttons no WhatsApp (fallback texto numerado). Reutilizar em auto-reply e fluxos que perguntem escolha ao cliente. |
| `OutboundSendError` / `messagingApiError`                                          | `server/whatsapp-service.ts` + `server/api-router.ts`           | Classificar falha de envio ao provider como `channel_disconnected`, `provider_rejected`, `provider_unavailable` ou `provider_timeout`, e traduzir para status HTTP e código estável. Rotas de mensagem não devem deixar falha de provider virar `internal_error`.                                      |
| `authorizeOutboundWhatsAppRequest` / `forwardZeloChatSendText`                     | `server/zelochat-internal-send.ts`                              | Gateway Outbound→Mend→ZeloChat Techne send. Auth `x-mend-outbound-key`; forward com a chave Chat já existente. Fail-closed. Não criar segunda chave no Chat nem reusar `MEND_API_TOKEN`.                                                                                                               |
| `formatHumanWhatsAppText` / `agentAttendingLine`                                   | `server/whatsapp-agent-intro.ts`                                | Prefixar a **primeira** resposta humana no WhatsApp com `_*Nome está te atendendo*_` (itálico+negrito) acima do corpo. Mensagens humanas seguintes vão sem intro; não aplicar em respostas da IA.                                                                                                      |
| `APP_SHORTCUTS` / `useAppShortcuts`                                                | `src/app/shortcuts/`                                            | Atalhos globais (`Cmd/Ctrl+K`, `/`, `C`, chords `G then …`). Registry único alimenta hints da command palette e o listener; não duplicar sequências hardcoded na UI. Ignorar quando o foco está em campo editável ou a palette/create estão abertos.                                                      |
| `ToastTone`                                                                        | `src/shared/ui/toast.ts`                                        | Distinguir aviso de sucesso e de falha no toast global. Falha usa ícone e `role="alert"`; nunca anunciar erro como sucesso.                                                                                                                                                                            |
