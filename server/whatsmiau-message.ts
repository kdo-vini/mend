/**
 * Whatsmiau/Baileys content classification shared by webhook normalization
 * and inbox persistence. Voice notes often arrive wrapped (view-once,
 * disappearing messages) or with only `messageType` + `base64`, which used
 * to be stored as an empty text bubble.
 *
 * Keep `supabase/functions/whats-mend-webhook/message-content.ts` in sync.
 */
...
export function unwrapProviderMessage(
...
export function providerContentType(
  message: Record<string, unknown>,
  hint?: string,
): ProviderContentType {
...
/** Reads one webhook data item, including wrapped voice notes and base64 media. */
...
/** Browser-fetchable media URL. Encrypted WhatsApp CDN links stay out of the inbox. */
